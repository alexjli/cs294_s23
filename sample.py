...
from src.diffusion import WhitenedDiffuser_IdealChain
...
if __name__ == '__main__':
    ...
    model_dir = os.path.join("models", "20230429_1")
    sample_dir = os.path.join(model_dir, "samples")
    if not os.path.exists(sample_dir):
        os.mkdir(sample_dir)
    checkpoint = torch.load(os.path.join(model_dir, "checkpoint_4.pt"))
    model = SE3Denoiser()
    diffuser = WhitenedDiffuser_IdealChain(model)
    ...
    save_fused_pdbs(xyzs, pdb_out=os.path.join(f"ideal_trajectory_2.pdb"))
